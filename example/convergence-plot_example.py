...
'''
Script to generate the png files with the SSSP convergence pattern plots for the test example (elemental palladium).

It needs:

- json files with SSSP pseudopotentials and cutoffs (precision and efficiency): available at https://www.materialscloud.org/discover/sssp/
...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, prog='sssp_convergence_plot',
    description="""
This AiiDA script generates the png files containing the SSSP convergence pattern plots for the Pd test example.
""")

parser.add_argument("sssp_lib", type=str, help="SSSP library: 'efficiency' or 'precision'")
parser.add_argument("wien2k_pk", type=int, help="pk of the SinglefileData of the 'Wien2k.txt' file from calcDelta package")
parser.add_argument("file_SSSP_efficiency", type=str, help="local path of json files with SSSP efficiency tables")
...
file_SSSP_efficiency = args.file_SSSP_efficiency 
file_SSSP_precision = args.file_SSSP_precision
####
...
with open(os.path.expanduser('~{}'.format(file_SSSP_precision)),'r') as o:
...
#                    'Dojo'
    ...
    groups_bands=Group.query(name='Pd_031PAW_8_sssp_bands')
...
#                             bbox_to_anchor=(-0.05, 1.0), fontsize=14, frameon=True, markerscale=1.0)            
        ...
        # Put circle at the corresponding SSSP pseudopotential and cutoff
